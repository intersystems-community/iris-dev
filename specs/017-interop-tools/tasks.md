...
- [x] T001 Create `crates/iris-dev-core/tests/interop_unit_tests.rs` with unit tests for all 9 tools: mock `IrisConnection` xecute/query responses; verify param parsing, response shaping, error codes. Include tests for:
...
- [x] T002 Create `crates/iris-dev-core/tests/interop_e2e_tests.rs` with e2e tests: spawn `iris-dev mcp`, send MCP tool calls. Include tests for:
...
- [x] T003 Run `cargo test --test interop_unit_tests --test interop_e2e_tests` — confirm ALL tests FAIL
...
- [x] T004 Create `crates/iris-dev-core/src/tools/interop.rs` with param structs for all 9 tools (serde Deserialize + schemars JsonSchema)
- [x] T005 Implement `interop_production_status` in `interop.rs`: xecute inline ObjectScript that calls `Ens.Director.GetProductionStatus()` and Writes JSON with production name + state string + state code; with `full_status`, iterate `Ens.Config.Item` for per-component breakdown
- [x] T006 [P] Implement `interop_production_start`: xecute `Ens.Director.StartProduction(name)`
- [x] T007 [P] Implement `interop_production_stop`: xecute `Ens.Director.StopProduction(timeout, force)`
- [x] T008 [P] Implement `interop_production_update`: xecute `Ens.Director.UpdateProduction(timeout, force)`
- [x] T009 [P] Implement `interop_production_needs_update`: xecute `Ens.Director.ProductionNeedsUpdate()` → boolean
- [x] T010 [P] Implement `interop_production_recover`: xecute `Ens.Director.RecoverProduction()`
- [x] T011 Register all 6 lifecycle tools in `src/tools/mod.rs` via `#[tool_router]`
- [x] T012 Run `cargo test --test interop_unit_tests` — lifecycle tool tests must PASS
...
- [x] T013 Implement `interop_logs`: query `SELECT ID, TimeLogged, Type, ConfigName, Text FROM Ens_Util.Log WHERE ...` with filters for item_name, log_type, limit
- [x] T014 [P] Implement `interop_queues`: query `SELECT * FROM Ens.Queue_Enumerate()`
- [x] T015 [P] Implement `interop_message_search`: query `SELECT ID, TimeCreated, SourceConfigName, TargetConfigName, MessageBodyClassName, Status FROM Ens.MessageHeader WHERE ...` with source/target/class filters
- [x] T016 Register all 3 observability tools in `src/tools/mod.rs`
- [x] T017 Run `cargo test --test interop_unit_tests` — ALL 9 tool unit tests must PASS
- [x] T018 Run `cargo test --test interop_e2e_tests` — e2e tests must PASS against iris-dev-iris (requires Ensemble enabled)
...
- [x] T019 Create or verify an Ensemble-enabled namespace in iris-dev-iris container
- [x] T020 Write a simple test production class (`Demo.TestProduction` with one File passthrough service) and compile into iris-dev-iris
- [x] T021 Run full demo flow: status → start → check logs → check queues → stop → verify status=Stopped → start → recover
- [x] T022 Verify all tool responses are structured JSON suitable for programmatic capture
- [x] T023 Time each tool response — verify all complete in <2s
- [x] T024 Record the demo flow as a script in `specs/017-interop-tools/demo-script.md`
