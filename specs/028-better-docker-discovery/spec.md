...
- **FR-001**: `discover_via_docker_named()` MUST distinguish and emit separate log messages for:
  ...
  - Mode (d): Container found, port mapped, probe fails (web server down) → `"Container '{name}' found at localhost:{port} but Atelier REST API is not responding — is the IRIS private web server running? (WebServer=1 in iris.cpf, httpd binary present?)"`
