...
fn mcp_exchange(env_vars: &[(&str, &str)], messages: &[serde_json::Value]) -> Vec<serde_json::Value> {
        ...
        if msg.get("id").is_some() {
            ...
            loop {
                ...
                if reader.read_line(&mut line).unwrap_or(0) > 0 {
                    if let Ok(v) = serde_json::from_str::<serde_json::Value>(&line) { results.push(v); break; }
                }
...
fn parse_tool(responses: &[serde_json::Value], id: u64) -> serde_json::Value {
    let resp = responses.iter().find(|r| r["id"] == id).expect("no response");
...
fn iris_env() -> Vec<(&'static str, String)> {
    vec![
        ("IRIS_HOST",     std::env::var("IRIS_HOST").unwrap_or_else(|_| "localhost".into())),
        ("IRIS_WEB_PORT", std::env::var("IRIS_WEB_PORT").unwrap_or_else(|_| "52780".into())),
        ("IRIS_USERNAME", std::env::var("IRIS_USERNAME").unwrap_or_else(|_| "SuperUser".into())),
        ("IRIS_PASSWORD", std::env::var("IRIS_PASSWORD").unwrap_or_else(|_| "SYS".into())),
    ]
}
...
fn iris_doc_put_no_scm() {
    ...
    assert!(result["open_uri"].as_str().is_some(), "open_uri should be present: {}", result);
    assert!(result["open_uri"].as_str().unwrap().starts_with("isfs://"), "open_uri should be isfs: {}", result);
}
...
fn iris_source_control_status_uncontrolled() {
    ...
    let result = parse_tool(&responses, 2);
    assert_eq!(result["success"], true, "status should not error: {}", result);
    ...
    assert!(result.get("controlled").is_some(), "controlled field must be present: {}", result);
}
...
fn iris_compile_open_uri() {
    ...
    assert_eq!(result["success"], true, "compile should succeed: {}", result);
    assert!(result["open_uri"].as_str().is_some(), "open_uri should be present after compile: {}", result);
