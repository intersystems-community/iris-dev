...
fn test_sweep_removes_expired_entries() {
    ...
    let id2 = store.insert("Doc2.cls", ElicitationAction::ScmExecute, None, Some("CheckOut".into()), "USER");
    ...
    assert!(removed == 0 || removed <= 2, "sweep on live entries removes 0 or some: {}", removed);
}
