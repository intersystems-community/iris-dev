...
impl IrisConnection {
    ...
    /// compile time, captures its `Write` output via a temp file, and embeds it as a static
    /// return value. The generated method is then called via SQL to retrieve the output.
    /// Finally the temp class is deleted. Requires write + compile permissions on the namespace.
    pub async fn execute_via_generator(
        ...
        // IRIS SQL function name: schema = "User", procedure = "IrisDevRun{id}_Execute"
        ...
        // 3. Query via SQL — the generated method returns the captured output
    ...
    fn build_exec_class(class_name: &str, tmpfile: &str, code: &str) -> Vec<String> {
        ...
        lines.extend([
            ...
            "  If $TEST {".into(),
            "    Set line = \"\"".into(),
            "    For  { Read line:0  If '$TEST Quit  Set out = out _ line _ $Char(10) }".into(),
            "    Close tmpfile".into(),
            "  }".into(),
    ...
    /// Delete an Atelier document (best-effort; ignores errors).
    async fn delete_doc(
        &self,
        doc_name: &str,
        namespace: &str,
